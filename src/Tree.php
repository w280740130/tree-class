...
class Tree {
    ...
    //递归实现获取指定ID的所有父类ID
    public function get_parents($catid='', $arrparentid='') {
        $pid = $this->arr[$catid]['pid'];
        $arrparentid = $arrparentid ? $pid.','.$arrparentid : $pid;
        if($pid) {
            $arrparentid = $this->get_parents($pid,$arrparentid);
        }
        return $arrparentid;
    }
    //递归实现获取指定ID的所有子ID
    public function get_childs($catid='') {
        $arrchildid = $catid;
        if (is_array($this->arr)) {
            foreach ($this->arr as $id => $cat) {
                if ($id != $catid && $cat['pid'] == $catid) {
                    $arrchildid .= ',' . $this->get_childs($id);
                }
            }
        }
        return $arrchildid;
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                ...
                if ($number == $total) {
                  ...
                  if($number==1){
                      if($first==0){
                        $this->arr_tree .= '<ul class="'.$class.'"'.$class.'"><li>';
                      }else{
                        $this->arr_tree .= '<ul><li>';
                      }
                  }else{
                ...
                $this->arr_tree .= $nstr;
                $nbsp = $this->nbsp;
               $re =  $this->get_tree_array($id, $str, $sid, $adds . $k . $nbsp, $str_group,1);
               if(empty($re)){
                   $this->arr_tree .= '</li>';
               }  else {
                   $this->arr_tree .= '</ul>';
